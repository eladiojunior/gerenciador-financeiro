...
namespace GFin.Dados
{
    internal class GenericDAO<T> : IGenericDAO<T> where T : class
    {
        ...
        public T ObterPorId(int id)
        {
            ...
            {
                throw new DAOException(string.Format("Erro ao obter registro pelo Id: {0}, entidade: {1}", id, typeof(T).Name), erro);
            }
        }
        ...
        public T Incluir(T entidade)
        {
            try
            {
                return DbContextoSet().Add(entidade);
            }
            catch (System.Data.Entity.Validation.DbEntityValidationException erro)
            {
                throw new DAOException(erro);
