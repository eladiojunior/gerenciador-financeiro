...
namespace GFin.Negocio
{
    ...
    {
        public ChequeNegocio(GFin.Negocio.Listeners.IClienteListener clienteListener) : base(clienteListener) { }
        /// <summary>
        ...
        public void RemoverCheque(Cheque cheque)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork())
            {
                ...
                {//Nenhuma despesa mensal vinculada... exclusão física.
                    throw new NegocioException(string.Format("Existem movitentações deste Cheque [{0}], sua remoção não é permitida. Favor realizar o cancelamento do Cheque.", cheque.NumeroCheque));
                }
        ...
        public void CancelarCheque(ChequeCancelado chequeCancelado)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork())
            {
                ...
                if (_cheque == null)
                {
                    throw new NegocioException(string.Format("Não foi possível encontrar o cheque pelo ID [{0}].", chequeCancelado.IdCheque));
                }
                if (_cheque.CodigoSituacaoCheque == (short)TipoSituacaoChequeEnum.ChequeCancelado)
                {
                    throw new NegocioException(string.Format("Número do Cheque [{0}] da Conta [{1}] já cancelado.", _cheque.NumeroCheque, _cheque.ContaCorrente.BancoAgenciaContaCorrente));
                }
        ...
        internal void EmitirCheque(ChequeEmitido chequeEmitido)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork())
            {
                ...
                if (_cheque == null)
                {
                    throw new NegocioException(string.Format("Não foi possível encontrar o cheque pelo ID [{0}].", chequeEmitido.IdCheque));
                }
                if (_cheque.CodigoSituacaoCheque == (short)TipoSituacaoChequeEnum.ChequeEmitido)
                {
                    throw new NegocioException(string.Format("Número do Cheque [{0}] da Conta [{1}] já emitido.", _cheque.NumeroCheque, _cheque.ContaCorrente.BancoAgenciaContaCorrente));
                }
        ...
        public void CancelarChequePorRemocaoDespesaMensal(int idCheque, string descricaoDespesa)
        {
            ...
            {//Cancelar cheque.
                ...
                chequeCancelado.ObservacaoCancelamentoCheque = string.Format("Cancelamento de cheque pela remoção de despesa que o emitiu. Descrição despesa: {0}.", descricaoDespesa);
