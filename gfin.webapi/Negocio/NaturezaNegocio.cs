...
namespace gfin.webapi.Negocio
{
    ...
    {
        public NaturezaNegocio(IClienteListener cliente, GFinContext context) : base(cliente, context) { }
        ...
        private void ValidarNatureza(NaturezaConta natureza)
        {
            ...
            if (!UtilEnum.IsEnumValido(typeof(TipoLancamentoEnum), natureza.CodigoTipoLancamentoConta))
            {
                throw new NegocioException(
                    $"Tipo da natureza [{natureza.CodigoTipoLancamentoConta}] inválido, informe 1-Despesa ou 2-Receita.");
            }
            ...
            {//Nova
                ...
                using (IUnitOfWork uofw = new UnitOfWork(GFinContext))
                {
                    var idEntidade = ClienteListener.UsuarioLogado.IdEntidade;
                    var qtdRegs = uofw.NaturezaConta.QuantRegistros(n => n.IdEntidade == idEntidade && 
                        n.CodigoTipoLancamentoConta == natureza.CodigoTipoLancamentoConta &&
                        n.DescricaoNaturezaConta == natureza.DescricaoNaturezaConta);
                    if (qtdRegs > 0)
        ...
        public void RemoverNaturezaConta(int idNatureza)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork(GFinContext))
            {
                ...
                if (_natureza == null)
                    throw new NegocioException(
                        $"Não foi possível encontrar a natureza da conta com o Id [{idNatureza}].");
