...
using gfin.webapi.Core.Negocio.DTOs;
...
namespace gfin.webapi.Negocio
{
    ...
    {
        public DespesaNegocio(IClienteListener cliente, GFinContext context) : base(cliente, context) { }
        ...
        public DespesaMensal RegistrarDespesa(DespesaMensal despesa)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork(GFinContext))
            {
                ...
                {//Gerar emissão do cheque.
                    ...
                    chequeEmitido.HistoricoEmissaoCheque =
                        $"{UtilEnum.GetTextoFormaLiquidacao(despesa.CodigoTipoFormaLiquidacao)} emitido para despesa: {despesa.DescricaoDespesa}";
                    ChequeNegocio negocioCheque = new ChequeNegocio(ClienteListener, GFinContext);
        ...
        public void RegistrarDespesaParcelada(DespesaMensalParcelada despesaParcelada)
        {
            using (IUnitOfWork uofw = new UnitOfWork(GFinContext))
            {
                ...
                foreach (var item in despesaParcelada.ParcelamentoDespesa)
	            {
                    ...
                    despesaMensal.DescricaoDespesa =
                        $"{item.NumeroParcela.ToString("00")}/{qtdParcelas.ToString("00")}-{despesaParcelada.TextoDescricaoDespesa}";
                    ...
                    {//Gerar emissão do cheque.
                        ...
                        chequeEmitido.HistoricoEmissaoCheque =
                            $"{UtilEnum.GetTextoFormaLiquidacao(despesaMensal.CodigoTipoFormaLiquidacao)} emitido para despesa: {despesaMensal.DescricaoDespesa}";
                        ChequeNegocio negocioCheque = new ChequeNegocio(ClienteListener, GFinContext);
                        negocioCheque.EmitirCheque(chequeEmitido);
                    }
        ...
        private void ValidarDespesa(DespesaMensal despesa)
        {
            ...
            {//Verificar se a natureza informada existe...
                throw new NegocioException(
                    $"Natureza da Despesa (Id: {despesa.IdNaturezaContaDespesa}) não existe ou está inativa.");
            }
        ...
        private void ValidarDespesaFixa(DespesaFixa despesa)
        {
            ...
            {//Verificar se a natureza informada existe...
                throw new NegocioException(
                    $"Natureza da Despesa Fixa (Id: {despesa.IdNaturezaContaDespesaFixa}) não existe ou está inativa.");
            }
        ...
        public void RemoverDespesaFixa(int idDespesaFixa)
        {
            ...
            using (IUnitOfWork uofw = new UnitOfWork(GFinContext))
            {
                ...
                if (_despesaFixa == null)
                    throw new NegocioException(
                        $"Não foi possível encontrar a despesa fixa com o Id [{idDespesaFixa}].");
                
                //Verificar vinculo em despesas mensais.
                var qtd = uofw.DespesaMensal.QuantRegistros(dm => dm.IdEntidade == ClienteListener.UsuarioLogado.IdEntidade && dm.DespesaFixa.Id == idDespesaFixa, dm => dm.DespesaFixa);
                hasVinculos = (qtd != 0);
                if (!hasVinculos)
                {//Verificar vinculo no histórico de despesas.
                    qtd = uofw.HistoricoDespesaFixa.QuantRegistros(q => q.IdDespesaFixa == idDespesaFixa);
                    hasVinculos = (qtd != 0);
                }
        ...
        public void RemoverDespesaMensal(int idDespesa, bool hasRemoverParcelamento)
        {
            ...
            if (_despesa == null)
                throw new NegocioException($"Não foi encontrado a despesa mensal com o Id [{idDespesa}] para remoção.");
        ...
        public void LiquidarDespesa(int idDespesa, DateTime dataLiquidacao, decimal? valorDesconto, decimal? valorJurosMulta, string observacao)
        {
            ...
            DespesaMensal _despesa = ObterDespesaMensal(idDespesa);
            if (_despesa == null)
                throw new NegocioException($"Despesa mensal com o Id [{idDespesa}] não encontrada para liquidação.");
            if (_despesa.IsDespesaLiquidada)
